...
for E_GB in E_GB_to_test: # energy of gap state from intrinsic level [eV]
    ...
    equilibrium = sesame.solve_equilibrium(sys)
    solution = sesame.solve(sys, equilibrium)

    sys.generation(lambda x, y: 1e17 )
    solution = sesame.solve(sys, solution)

    sys.generation(lambda x, y: 1e19 )
    solution = sesame.solve(sys, solution)

    sys.generation(lambda x, y: 1e20 )
    solution = sesame.solve(sys, solution)

    sys.generation(lambda x, y: 1e21 )
    solution = sesame.solve(sys, solution)

    #newsys = makesystem(rho_GB[1])
    #solution = sesame.solve(newsys, solution)


    print(E_GB)
    analyze = sesame.Analyzer(sys, solution)
