...
def getF(sys, v, efn, efp, veq, periodic = True):
    ...
    # currents
    jnx_sm1 = get_jn(sys, efn, v, sites - 1, sites, dxm1)
    jny_s = get_jn(sys, efn, v, sites, (sites + Nx) % Nx, dy)
    jny_smN = get_jn(sys, efn, v, (sites - Nx) % Nx, sites, dym1)

    jpx_sm1 = get_jp(sys, efp, v, sites - 1, sites, dxm1)
    jpy_s = get_jp(sys, efp, v, sites, (sites + Nx) % Nx, dy)
    jpy_smN = get_jp(sys, efp, v, (sites - Nx) % Nx, sites, dym1)
