...
def getFandJ_eq(sys, v, periodic_bcs):
    ...
    def laplacian(vsmN, vsm1, vs, vsp1, vspN, dxm1, dx, dym1, dy, dxbar, dybar): #TODO why is this here?
    ...
    dybar = (dy + dym1) / 2.
    ### bottom
    # dy = np.append(dy, np.repeat(sys.dy[0], Nx - 2))
    # dym1 = np.append(dym1, np.repeat((sys.dy[0] + sys.dy[-1]) / 2, Nx - 2))
    ### top
